...
if (-not $Force -and (Test-Path -LiteralPath $OutputPath -PathType Leaf)) {
    ...
    if ($newest -lt (Get-Item -LiteralPath $OutputPath).LastWriteTime) {
        Write-Host "SD image is up to date: $OutputPath"
        return
    }
}
...
# QEMU requires an SD image whose complete byte length is a power of two. Start
...
if ($clusters -lt $script:MinClusters -or $clusters -lt $neededClusters) {
    throw 'The requested scenes do not fit in the maximum supported 32 GiB QEMU SD image.'
}
...
Write-Host (
    'Building SD image for {0} scene(s), {1} MiB data, {2} MiB volume: {3}' -f
    $files.Count,
...
try {
    ...
    $stream.Flush()
    Write-Host 'SD image build complete.'
}
finally {
    $stream.Dispose()
}
