...
- a downloaded DMDClock scene library or your own compatible `.scn` files if you
  want animations
...
1. Download the `DMDClock-…-win-x64-standalone.zip` asset from the project's
   [GitHub Releases page](https://github.com/DrWize/DMD-Pinball-Clock-Win-x64-and-ESP32/releases).
...
3. Choose **DMD-Large** (preferred, 2,416 scenes) or **Original DotCLK-Orig**
   (2,324 scenes), review its source, and select **Download**.
...
The app installs only verified `.scn` files under a managed directory such as:

```text
%LOCALAPPDATA%\DmdClock\Scenes\DMD-Large\
%LOCALAPPDATA%\DmdClock\Scenes\DotCLK-Orig\
```

Downloads are limited in size, ZIP paths and checksums are verified before
activation, and an installed pack is replaced only after the complete new pack is
ready. Scenes remain outside the EXE and SCR.

You can also [browse the original DotClk Scenes directory](https://github.com/sigmafx/DotClk-Resources/tree/master/Scenes).
DMDClock does not own the animation collections; review each source repository's
terms before redistributing its files.
