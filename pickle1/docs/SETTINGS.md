...
| **Download DotClk scenes…** | Opens the verified scene-library chooser. **DMD-Large** is preferred; **Original DotCLK-Orig** remains available. Each pack is installed under its own directory below `%LOCALAPPDATA%\DmdClock\Scenes\`. |
...
the [font reference](development/reference/FONTS.md) for the source commit, hashes,
