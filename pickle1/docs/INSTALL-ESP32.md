...
versions of the entry scripts directly from the `master` branch:

```powershell
Invoke-WebRequest 'https://raw.githubusercontent.com/DrWize/DMD-Pinball-Clock-Win-x64-and-ESP32/master/scripts/esp32/RUNME-Install-DmdClockEsp32.ps1' -OutFile 'RUNME-Install-DmdClockEsp32.ps1'
...
to date), then flashes the board from the staged payload. The direct
