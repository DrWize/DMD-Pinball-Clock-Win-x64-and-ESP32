...
Current stable release: **v1.6.0**. Supported embedded targets are the original
Waveshare ESP32-S3-Touch-LCD-7 (800×480, N16R8) and the 3.49B V2 /
Rev1.1 (640×172, N16R8). 

## Shared

### Release validation and robustness
...
- [X] Preserve the 128×32 DMD aspect ratio, resize tiles to fill the available window,
...
### Shared catalog and product language
...
- [ ] In the "Flash the firmware" step, list the USB drivers needed for each board
      and where to get them (for example the WCH CH343 driver for the 7" board)
      in the prerequisites section

### Non-destructive acceptance matrix and documentation

- [x] Add verifiable tests for Windows 11 x64 with PowerShell 7; no microSD card; no
...
- [x] Make the primary acceptance gate prove that a normal user can run staging on
      Windows 11 x64 / PowerShell 7 with no microSD card and no ESP32 attached and obtain
      every required local artifact without formatting a disk or modifying
      hardware.
- [x] Test destructive command construction only through mocks, fixtures,
...
### Windows-prepared ESP32 microSD card
...
- [ ] Cover catalog/archive errors, cancellation, resume, corrupt data, removed or
      full microSD cards, power loss during every phase, rollback, custom-scene
...
- [ ] Bound and rotate log storage, fall back to a RAM ring buffer when the microSD card
...
- [ ] Cover first-login enforcement, password reset, unauthorized access, failed/
      interrupted downloads, full or removed microSD cards, log rotation, and live-log
...
- [ ] Measure total and per-core CPU utilization plus per-task FreeRTOS run time;
      ...
      Wi-Fi RSSI, microSD card state, reset reason, and NTP state
...
- [ ] Check the web remote page width on both mobile and computer screens and make
      sure the layout fits without horizontal scrolling
- [ ] Investigate the Weekly screen schedule grid: hours from 22 onwards seem to
      be missing / not selectable in the schedule
