...
# (via New-DmdClockQemuSdImage.ps1) that QEMU attaches as the microSD card, so the
...
if (-not [string]::IsNullOrWhiteSpace($ScenesFolder) -or
    ...
    if (-not (Test-Path -LiteralPath $imagePath -PathType Leaf)) {
        throw "microSD image not found: $imagePath"
    }
    ...
    Write-Host "Attaching microSD card image: $imagePath"
}
if (-not $SkipBuild) {
